import { MarketplaceService } from '../systems/marketplace/services/marketplace.service';
...
async function main() {
    ...
    if (order.status !== 'pending_payment') console.error('FAIL: Status should be pending_payment');
    if (!order.expiryTime) console.error('FAIL: Expiry time not set');
    ...
    if (updatedOrder2?.status !== 'paid') console.error('FAIL: Order 2 should be paid');
    ...
    if (updatedOrder3?.status !== 'cancelled') console.error('FAIL: Order 3 should be cancelled');
