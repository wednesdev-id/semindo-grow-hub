...
export const getRevenueSummary = async (req: Request, res: Response) => {
    try {
        ...
        const completedRequests = await prisma.consultationRequest.findMany({
            where: {
                status: 'completed',
                ...dateFilter
            },
        ...
        const totalRevenue = completedRequests.reduce(
            (sum, r) => sum + parseFloat(r.quotedPrice.toString()), 0
        );
        ...
        const revenueByConsultant: Record<string, { name: string; revenue: number; sessions: number }> = {};
...
export const getRevenueTrends = async (req: Request, res: Response) => {
    try {
        ...
        // Group by date
        const trendsByDate: Record<string, { revenue: number; sessions: number }> = {};
...
export const getKPIMetrics = async (req: Request, res: Response) => {
    try {
        ...
        const [
            ...
            prisma.consultationRequest.count({ where: { status: 'completed' } }),
            prisma.consultationRequest.count({ where: { status: 'cancelled' } }),
        ...
        res.json({
            kpis: {
                consultantActivationRate: parseFloat(consultantActivationRate as string),
                ...
                requestGrowth: parseFloat(requestGrowth as string)
            },
            counts: {
                totalConsultants,
                activeConsultants,
