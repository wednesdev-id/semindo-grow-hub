...
export class UMKMService {
    async getStats() {
        ...
        const verified = await prisma.uMKMProfile.count({ where: { status: 'verified' } });
        const unverified = await prisma.uMKMProfile.count({ where: { status: 'unverified' } });
        const byLevel = await prisma.uMKMProfile.groupBy({
            by: ['level'],
            _count: { level: true },
        });
    ...
    async create(userId: string, data: Prisma.UMKMProfileCreateInput) {
    ...
    async update(id: string, data: Prisma.UMKMProfileUpdateInput) {
        ...
        if (data.turnover || data.employees || data.legalStatus) {
            ...
            const current = await prisma.uMKMProfile.findUnique({ where: { id } });
            if (current) {
    ...
    async delete(id: string) {
        return prisma.uMKMProfile.delete({
            where: { id },
        });
    }

    private calculateSegmentation(data: any): { level: string; reason: string } {
