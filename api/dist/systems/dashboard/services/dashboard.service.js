...
class DashboardService {
    async getOverview(userId) {
        ...
        const store = await db_1.db.store.findFirst({ where: { userId } });
        ...
        const [totalProductsCount, activeOrdersCount, enrollmentsCount, completedEnrollmentsCount] = await Promise.all([
        ...
        const completedOrders = await db_1.db.order.findMany({
            where: {
                ...orderWhereInput,
        ...
        let averageRating = 0;
        const reviewsAggregate = await db_1.db.review.aggregate({
            where: {
                OR: [
                    storeId ? { storeId } : {},
    ...
    async getAdminOverview() {
        try {
            ...
            const [totalUsers, totalUMKMProfiles, pendingUMKMVerifications, totalEnrollments, totalOrders, totalCourses] = await Promise.all([
                db_1.db.user.count(),
                db_1.db.uMKMProfile.count(),
                db_1.db.uMKMProfile.count({
                    where: {
                        status: { in: ['submitted', 'in_review'] }
                    }
                }),
                db_1.db.enrollment.count(),
                db_1.db.order.count(),
                db_1.db.course.count()
            ]);
            // Calculate active users (users with recent activity)
            // Using a simplified approach: users who have enrollments or orders
            const activeUsersFromEnrollments = await db_1.db.enrollment.groupBy({
                by: ['userId'],
                _count: { userId: true }
            });
            const activeUsersFromOrders = await db_1.db.order.groupBy({
                by: ['userId'],
                _count: { userId: true }
            });
            ...
            const pendingVerifications = await db_1.db.uMKMProfile.findMany({
                where: {
                    status: { in: ['submitted', 'in_review'] }
                },
                include: {
    ...
    async getSalesChartData(userId, storeId) {
        try {
    ...
    calculateSystemHealth(metrics) {
        ...
        if (metrics.totalUMKMProfiles > 50)
            score += 2;
        if (metrics.totalUMKMProfiles > 200)
            score += 2;
        // Bonus for learning activity
        if (metrics.totalEnrollments > 100)
            score += 1;
        if (metrics.totalCourses > 10)
            score += 1;
