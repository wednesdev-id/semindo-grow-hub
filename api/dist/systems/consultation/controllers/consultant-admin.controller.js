...
const getActiveConsultants = async (req, res) => {
    try {
        ...
        const consultantsWithMetrics = consultants.map(consultant => {
            ...
            const recentSessions = consultant.consultationRequests.filter(r => new Date(r.createdAt) >= thirtyDaysAgo).length;
            return {
                id: consultant.id,
...
exports.getActiveConsultants = getActiveConsultants;
/**
...
const getConsultantPerformance = async (req, res) => {
    try {
        ...
        const now = new Date();
        const getLast30Days = (days) => {
            const date = new Date(now);
            date.setDate(date.getDate() - days);
            return date;
        };
        const last30DaysRequests = consultant.consultationRequests.filter(r => new Date(r.createdAt) >= getLast30Days(30));
        const last60DaysRequests = consultant.consultationRequests.filter(r => new Date(r.createdAt) >= getLast30Days(60));
        const last90DaysRequests = consultant.consultationRequests.filter(r => new Date(r.createdAt) >= getLast30Days(90));
        // Rating distribution
...
exports.updateConsultantStatus = updateConsultantStatus;
