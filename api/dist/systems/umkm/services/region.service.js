...
class RegionService {
     ...
     */
    async getProvinces() {
        const result = await prisma_1.prisma.uMKMProfile.groupBy({
            by: ['province'],
     ...
     */
    async getProvinceStats(province) {
        const where = {
            province: { contains: province, mode: 'insensitive' },
        };
        const [totalUMKM, bySegmentation, byLevel, avgMetrics, verifiedCount,] = await Promise.all([
            prisma_1.prisma.uMKMProfile.count({ where }),
            prisma_1.prisma.uMKMProfile.groupBy({
                by: ['segmentation'],
            ...
            prisma_1.prisma.uMKMProfile.aggregate({
                where,
                _avg: {
                    turnover: true,
                    employees: true,
            ...
            prisma_1.prisma.uMKMProfile.count({
        ...
        return {
            province,
            totalUMKM,
            bySegmentation: Object.fromEntries(bySegmentation.map(s => [s.segmentation || 'Unknown', s._count.segmentation])),
            byLevel: Object.fromEntries(byLevel.map(l => [l.level || 'Unknown', l._count.level])),
     ...
     */
    async getCityStats(province, city) {
        const where = {
            province: { contains: province, mode: 'insensitive' },
    ...
    async getMapData(params) {
        ...
        const profiles = await prisma_1.prisma.uMKMProfile.findMany({
            where,
